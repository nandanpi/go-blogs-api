...
type Service interface {
	...
	GetPost(c *gin.Context, id uint) *BlogPost
	GetAllPosts(c *gin.Context) []BlogPost
	UpdatePost(c *gin.Context, req types.UpdatePostRequest)
	DeletePost(c *gin.Context, id uint)
	CreateComment(c *gin.Context, req types.CreateCommentRequest)
	GetCommentByID(c *gin.Context, commentID uint) Comments
	GetComments(c *gin.Context, postID uint) []Comments
	DeleteComment(c *gin.Context, id uint)
